...
@objc(InAppStoryFavorites)
class InAppStoryFavorites: RCTViewManager {
  var storyView: StoryView!
  override func view() -> (FavoritesView) {
    let rect = CGRect(x: 0, y: 0, width: 300, height: 150);
    let v = FavoritesView()
    storyView = StoryView(frame: rect, feed: "default", favorite: true)
    storyView.target = {
          let vc = UIApplication.shared.firstKeyWindow?.rootViewController
